    ...
    public static class IsoperiaBuild
    {
        ...
        private static void WriteBuildReport(UnityEditor.Build.Reporting.BuildSummary summary)
        {
            try
            {
                ...
                sb.AppendLine("build_id:          " + PlayerSettings.bundleVersion);
                ...
                // Confirms the fix for the stale-PWA bug: the service worker's own
                // cache-busting constant must contain this exact build_id, or a
                // client who already loaded the game will keep serving whatever it
                // cached on its first visit no matter how many times we redeploy.
                sb.AppendLine();
                sb.AppendLine("## Service worker cache-bust check");
                string swPath = Path.Combine(BuildOutput, "ServiceWorker.js");
                if (File.Exists(swPath))
                {
                    bool stamped = File.ReadAllText(swPath).Contains(PlayerSettings.bundleVersion);
                    sb.AppendLine("service_worker_stamped: " + (stamped
                        ? "yes"
                        : "NO -- CACHE_VERSION does not contain build_id " + PlayerSettings.bundleVersion +
                          " -- clients will not see this build"));
                }
                else
                {
                    sb.AppendLine("service_worker_stamped: NO -- ServiceWorker.js not found in build output");
                }

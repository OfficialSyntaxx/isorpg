...
The Unity bridge went stale during the external asset refresh. The exact Unity process remained running, but `mcpforunity://editor/state` reported `stale_status` and `Editor.log` reached 39 consecutive MCP command TCS timeouts. Do not claim a fresh Unity reimport, Play Mode, route traversal, captures, or device/performance evidence from this checkpoint. Reconnect the exact instance, reimport the generated assets, scan Console/import metadata, then run the remaining M0-04 review. Keep the result PARTIAL/In review, preserve the four editor-only/unresolved BuildPipeline assertions, and do not start M0-05/M0-06.

Recovery succeeded on 2026-09-04. `unity@931634bd` now responds for `/Users/syntaxx/isorpg-m0/unity` on Unity 6000.5.8f1. Reimport of all 21 regenerated FBXs passed the Inspector scan: no missing materials, five TerrainLayers, shared TerrainData/TerrainCollider, 24 LODGroups, and `globalScale=100`. Play Mode had zero Unity errors and retained the five heroes, 24 scatter objects, wind test, foam ribbon, player, and camera. The next task is still visual/runtime proof: traverse the route and verify spring-arm collision, then capture only genuinely verified Beach_Wreck, Switchback, and Clifftop views. Do not reuse generic screenshots; do not claim device or authoring-hour evidence.

The controlled route probe then confirmed a blocker: the beach start was not grounded and the Wreck waypoint was diverted by collision geometry before reaching the authored wreck position. Later terrain samples were grounded, but complete route traversal and spring-arm no-clipping are not proven. Do not create or relabel captures until the collision/path issue is repaired and landmark views are genuinely verified.

The inspection spawn setup is now deterministic: `M0InspectionBootstrap.Start` samples Terrain height and the saved scene stores authored X/Z `(10.5,10.5)` with Y `0`. Play Mode still exposes a beach collision defect—the controller falls to approximately y=-316 despite a TerrainCollider raycast hit at y=1.381. Repair this CharacterController/TerrainCollider interaction within M0 scope before attempting traversal or captures; do not disable required colliders.

Diagnostics confirm valid TerrainCollider bounds/data, enabled layer collision, and successful manual downward CharacterController.Move. The failure occurs only with the active M0InspectionMotor/startup path; delayed placement and controller re-registration did not resolve it and were removed. Instrument the motor's first-frame Move/vertical velocity next, then repair that lifecycle without bypassing TerrainCollider.
The temporary motor diagnostics are complete and removed. Frames 1–10 at the terrain-derived beach spawn recorded zero input/displacement and `grounded=True` with `CollisionFlags.Below`; controlled traversal reached Beach, Wreck, Switchback, Plateau, and Clifftop. Spring-arm raycasts found no terrain/hero obstruction. Exactly three visually inspected captures are staged under `Assets/Screenshots/`: `Beach_Wreck.png`, `Switchback.png`, and `Clifftop.png`. Keep device, pinch, percentile, and authoring-hour limitations explicit.
M0-05 composition pass added three scene-local anchors under `M0 Inspection Mode/M0-05 Composition` and a warm Sun balance. The exact three anchor captures are `Beach_Wreck.png`, `Switchback.png`, and `Clifftop.png`; they are Editor evidence and retain the inspection UI overlay. No new assets or atlas changes were made. Device, pinch, percentile-performance and authoring-hour evidence remain NOT RUN; do not begin M0-06.

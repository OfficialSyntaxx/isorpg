...
def finalize(parts, name, make_lod=False):
    deselect()
    for part in parts:
...
], "shorelands_cliff_wall")
finalize([torus((0, 0, 5.2), 5.0, 1.15)], "shorelands_sea_arch")
finalize([
    ico((-3.0, 0, 3.1), (3.6, 2.6, 2.6)),
    ico((0, 0, 4.2), (3.8, 2.1, 1.8)),
    ico((3.0, 0, 3.0), (2.5, 2.0, 2.6)),
], "shorelands_plateau_overhang")
finalize([
    ico((-2.0, 0, 2.0), (3.3, 2.4, 2.0)),
    ico((2.2, 0, 1.6), (2.8, 2.1, 1.6)),
], "shorelands_wreck_rock_shelf")
finalize([torus((0, 0, 4.2), 4.0, 1.25)], "shorelands_cave_mouth")
...
for rock, sx, sy, sz in [
    ...
    finalize([ico((0, 0, sz), (sx, sy, sz), 3)], rock, True)
finalize([
